...
using TiltFive; // Include this to use TiltFive's API
...
namespace IKProject3
{
    public class JointController : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // Initialize the TiltFive Wand
            wand = TiltFiveManager.Instance.GetWand(Wand.Right);
        }

        // Update is called once per frame
        void Update()
        {
            if (wand.GetButtonDown(WandButton.Trigger))
            {
                RaycastHit hit;
                Ray ray = Camera.main.ScreenPointToRay(wand.GetPosition());
                if (Physics.Raycast(ray, out hit))
                {
                    if (hit.transform.gameObject == TCP)
                    {
                        screenPoint = Camera.main.WorldToScreenPoint(TCP.transform.position);
                        offset = TCP.transform.position - Camera.main.ScreenToWorldPoint(new Vector3(wand.GetPosition().x, wand.GetPosition().y, screenPoint.z));
                    }
                }
            }

            if (wand.GetButton(WandButton.Trigger))
            {
                Vector3 curScreenPoint = new Vector3(wand.GetPosition().x, wand.GetPosition().y, screenPoint.z);
